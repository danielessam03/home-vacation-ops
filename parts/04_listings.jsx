    ...
    const ListingsPage = () => {
      ...
      const rows = useMemo(() => {
        ...
        return data.listings.map((l) => ({ l, s: slaOf(l, cfg.sla_hours, now) })).filter(({ l, s: sl }) => {
          ...
          if (s && !(`${l.reference_code} ${l.title || ''} ${l.source_name || ''}`.toLowerCase().includes(s))) return false;
          return true;
      ...
      const exportCsv = () => downloadCSV(`hv-listings-${ymd(new Date())}.csv`,
        ['reference_code', 'title', 'status', 'location', 'property_type', 'deal_type', 'price', 'currency', 'completeness_pct', 'missing_fields', 'date_received', 'date_published_claimed', 'date_published_verified', 'hours', 'sla_state', 'source_type', 'source_name', 'entered_by', 'website_url'],
        rows.map(({ l, s }) => [l.reference_code, l.title, l.status, l.location, l.property_type, l.deal_type, l.price, l.currency, l.completeness_pct, l.missing_fields, l.date_received, l.date_published_claimed, l.date_published_verified, r1(s.hours), s.state, l.source_type, l.source_name, nameOf(l.entered_by), l.website_url]));
      ...
      return (
        <div>
          ...
          <div className="no-print mb-3 flex gap-2">
            <input className={inputCls()} placeholder="Search ref code, title or source…" value={q} onChange={(e) => setQ(e.target.value)} />
            ...
            <>
              ...
              <Card className="scroll-x hidden md:block">
                <table className="w-full text-sm">
                  <thead className="bg-slate-50 text-left text-xs text-slate-500"><tr>{['Ref code', 'Title', 'Status', 'SLA', 'Complete', 'Price', 'Source', 'Entered by', 'Received'].map((h) => <th key={h} className="px-3 py-2 font-medium">{h}</th>)}</tr></thead>
                  <tbody>
                    {rows.map(({ l, s }) => (
